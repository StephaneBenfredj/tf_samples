# vsphere provider
#
# Create Avi controller in vCenter using an existing vm template
...
#use an existing folder, otherwise need a ressource
...
# alternative with regular template
data "vsphere_virtual_machine" "template" {
  name       = var.avi_controller_vm_template
  datacenter_id = data.vsphere_datacenter.dc.id
}
...
resource "vsphere_virtual_machine" "avicontroller-vm" {
  ...
  # ovf_deploy { #seems to require host system ID !!
  #   remote_ovf_url = "http://10.206.112.50/controller-20.1.3-9085.ova"
  # }
